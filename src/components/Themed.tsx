...
export function BottomTextInput (props: TextInputPropsD): ReactElement {
  const { style, lightColor, darkColor, ...otherProps } = props;
  const border: string = useThemeColor({ light: lightColor, dark: darkColor }, 'text');
  const text: string = useThemeColor({ light: lightColor, dark: darkColor }, 'text');

  return (
    <DefaultTextInput className='border-b-2 w-56' placeholderTextColor={text} style={[{ color: text, borderBottomColor: border }, style]} {...otherProps} />
  )
}
...
export function InfiniteLoading (props: LoadingProps): ReactElement {
  const { lightImage, darkImage, style } = props;
  const colorScheme: ColorSchemeName = useColorScheme();
  const [customImage, setCustomImage] = useState(Images.icons.infinityLoadingDark);
  useEffect((): void => {
    if (colorScheme === "dark") {
      setCustomImage(darkImage);
    } else {
